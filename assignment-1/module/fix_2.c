                                   ...
                                   const char *buf, size_t count)
{
	...
	//sysfs__write_int(&command, (int)buf);
	//printk(KERN_INFO " %d - %d ",*buf, command);
        return count;
}
...
static int fault_hook(struct mm_struct *mm, struct pt_regs *regs, unsigned long error_code, unsigned long address)
{
   /*TODO Fault handler*/
    if(command == 0)
    {
	printk(KERN_INFO "gptr  [%lx] address [%lx] \n", gptr, address);
	...
	if( gptr <= address && vma_end > address){
		...
		if( error_code & 0x2 )
			...
			readwss += 1;		
		if ( error_code != 0xd && error_code != 0xf)
			printk(KERN_INFO " Ohh... What is this ? %lx", error_code );
		stac();
                gpte = get_pte(address, &vmaa);
                gpte->pte &= ~(0x1UL << 50);
                ptr  = (unsigned long *)address;
...
ssize_t handle_read(char *buff, size_t length)
{
   ...
   if(command == 0)
   {
		...
		{	unsigned long vmaa;
			...
			printk(KERN_INFO " gpte entry [%lx] ",gpte->pte);
                	gpte->pte |= 0x1UL << 50;
...
//			printk(KERN_INFO "[%lx] -- %d --%d -- [%lx] ",iterator, find_page_index(iterator), tlb_misses_list[find_page_index(iterator)], vma_end );
       ...
       if( cmd->command == TLBMISS_TOPPERS ){
		...
		for( ; i < num_pages ; i++)
		{
			if (tlb_misses_list[i] == 0)
				continue;
			else{
				if( tlb_misses_list[i] >= tlb_misses_list[max_0] ){
					max_4 = max_3;
					max_3 = max_2;
					max_2 = max_1;
					max_1 = max_0;
				...
				else if ( tlb_misses_list[i] >= tlb_misses_list[max_1] ){
					max_4 = max_3;
                                        max_3 = max_2;
                                        max_2 = max_1;
                                        max_1 = i;
				}
				else if ( tlb_misses_list[i] >= tlb_misses_list[max_2] ){
					max_4 = max_3;
                                        max_3 = max_2;
                                        max_2 = i;
				}
				else if ( tlb_misses_list[i] >= tlb_misses_list[max_3]){
					max_4 = max_3;
					max_3 = i;
				}
				else if ( tlb_misses_list[i] > tlb_misses_list[max_4]){
					max_4 = i;
				}
		...
		if ( tlb_misses_list[max_0] !=0 ){
			cmd->valid_entries += 1;
			cmd->toppers[0].vaddr = gptr + max_0 * 4096;
			cmd->toppers[0].count = tlb_misses_list[max_0];
		}
		if ( tlb_misses_list[max_1] !=0 && max_1 != max_0 ){
                        cmd->valid_entries += 1;
                        cmd->toppers[1].vaddr = gptr + max_1 * 4096;
                        cmd->toppers[1].count = tlb_misses_list[max_1];
                }
		if ( tlb_misses_list[max_2] !=0 && max_2 != max_1 ){
                        cmd->valid_entries += 1;
                        cmd->toppers[2].vaddr = gptr + max_2 * 4096;
                        cmd->toppers[2].count = tlb_misses_list[max_2];
                }
		if ( tlb_misses_list[max_3] !=0 && max_3 != max_2){
                        cmd->valid_entries += 1;
                        cmd->toppers[3].vaddr = gptr + max_3 * 4096;
                        cmd->toppers[3].count = tlb_misses_list[max_3];
                }
		if ( tlb_misses_list[max_4] !=0 && max_4 != max_3){
                        cmd->valid_entries += 1;
                        cmd->toppers[4].vaddr = gptr + max_4 * 4096;
                        cmd->toppers[4].count = tlb_misses_list[max_4];
                }
...
ssize_t handle_write(const char *buff, size_t lenth)
{
   ...
   clac();
   printk(KERN_INFO "Inside hook.c In write %lx\n", gptr);
   return 8;
...
int handle_open(void)
{
   /*TODO open handler*/
	...
	temp = (struct vma_area_list *)kmalloc(sizeof(struct vma_area_list),GFP_KERNEL);
        temp = head;
        while(temp->next!=NULL) {
	    printk(KERN_INFO " [%lx] - [%lx] ", temp->start, temp->end );
            temp = temp->next;
        }
